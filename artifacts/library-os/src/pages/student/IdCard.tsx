...
function CardFront({ flip }: { flip: () => void }) {
  return (
      ...
      <div className="relative p-5 h-full flex flex-col">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-bold text-white leading-tight">{currentStudent.library}</p>
              <p className="text-xs text-white/50 leading-tight">Student ID Card</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs text-sky-300 font-mono font-bold">{currentStudent.id}</p>
            <p className="text-xs text-white/40">Valid till {currentStudent.expiryDate}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div className="w-16 h-16 rounded-xl bg-white/10 border-2 border-white/20 flex items-center justify-center flex-shrink-0">
            <span className="text-2xl font-black text-white/80">{currentStudent.name.split(" ").map(n => n[0]).join("")}</span>
          </div>
          <div>
            <p className="text-lg font-black text-white leading-tight">{currentStudent.name}</p>
            <p className="text-xs text-sky-300 font-medium mt-0.5">{currentStudent.membership} Member</p>
            <div className="flex items-center gap-2 mt-2">
              <span className="flex items-center gap-1 text-xs text-white/60"><MapPin className="w-3 h-3" /> Seat {currentStudent.seat}</span>
              <span className="w-1 h-1 bg-white/30 rounded-full" />
              <span className="text-xs text-white/60">Since {currentStudent.joinDate}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-end gap-4 mt-auto">
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 gap-1.5 text-xs">
              {[
                { l: "Membership", v: currentStudent.membership },
                { l: "Status", v: "Active" },
                { l: "Seat No.", v: currentStudent.seat },
                { l: "Library", v: "ReadSpace Pro" },
              ].map((d, i) => (
...
function CardBack({ flip }: { flip: () => void }) {
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-slate-800 via-slate-900 to-indigo-950 rounded-2xl overflow-hidden select-none cursor-pointer" onClick={flip}>
      ...
      <div className="relative p-5 h-full flex flex-col">
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 mb-3">
          <p className="text-xs font-bold text-red-400 mb-1">Emergency Contact</p>
          <p className="text-sm font-semibold text-white">{currentStudent.emergencyContact}</p>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-3 mb-3">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 text-xs text-white/70">
              ...
              <span>{currentStudent.library}</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-white/70">
              <MapPin className="w-3 h-3 flex-shrink-0" />
              <span>{currentStudent.libraryAddress}</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-white/70">
              <Phone className="w-3 h-3 flex-shrink-0" />
              <span>{currentStudent.libraryPhone}</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-4">
          ...
          <div className="bg-card border border-card-border rounded-2xl p-6 flex flex-col items-center">
            ...
            <p className="text-xs text-muted-foreground mt-3 text-center">Show this QR code at the reception for quick check-in / check-out</p>
            <Button
          ...
          <div className="bg-card border border-card-border rounded-xl p-4 space-y-2">
            ...
            {[
              ...
              { label: "Student ID", value: currentStudent.id },
              { label: "Membership", value: `${currentStudent.membership} — Active` },
              { label: "Seat", value: currentStudent.seat },
              { label: "Issued By", value: currentStudent.library },
              { label: "Valid Till", value: currentStudent.expiryDate },
            ].map((d, i) => (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} transition={{ type: "spring", stiffness: 200 }} className="flex flex-col items-center gap-6 z-10">
              ...
              <div className="text-center">
                <p className="text-white text-2xl font-black tracking-tight">{currentStudent.name}</p>
                <p className="text-white/50 text-sm font-mono mt-1">{currentStudent.id}</p>
                <p className="text-sky-400 text-sm font-medium mt-1">Seat {currentStudent.seat} · {currentStudent.membership} Plan</p>
              </div>
